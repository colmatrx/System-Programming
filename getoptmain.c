...
int main(int argc, char *argv[]){
    ...
    if (argc > 3){                                      //using argc to test the number of command line arguments
        ...
        //call help function here to display usage
        exit (0);
    }
    ...
    /*else if ((argc == 2) && (argv[1][0] != '-'))
            printf("\nYou have provided filename as %s.log\n", argv[1]);*/ //This block can also be used to print the filename provided like the optind below
       ...
       printf("\nYou have provided filename as %s.log\n", argv[optind]); //optind is the index of the next character not preceded by an '-' so we can use it to print the filename
